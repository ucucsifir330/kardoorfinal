...
/**
 * EntranceLab — yeni hero sahnesinin geliştirme zemini  (route: /entrance-lab)
 *
 * Şu anki kapsam:
 ...
 *  • Kapının arkasındaki siyah boşluk hero'da hazır → kapı açılınca karanlık
 *    iç görünür. (Showroom/turntable sonraki adımda buraya gelecek.)
 *
 * Mimari:
...
const updateMaster = (raw: number) => {
  ...
  const section = sectionRef.value;
  if (section) {
    section.style.setProperty("--page-x", `${slideP * -100}%`);
...
onMounted(() => {
  ...
  door.load(doorMeta.value).catch((error) => {
    console.error("[EntranceLab] Kapı sprite yüklenemedi.", error);
  });
...
useSeoMeta({
  title: "Entrance Lab",
  description: "Hero yeniden yazım zemini — tek background görsel + kapı sprite."
});
</script>
        ...
        <div class="entrance-lab__slide entrance-lab__configure">
          <div class="entrance-lab__configure-inner">
            ...
            <div class="entrance-lab__configure-actions">
              <a href="/catalog" class="ada-manifesto-cta entrance-lab__soon-cta" aria-label="Konfigüratörü deneyin — çok yakında">
