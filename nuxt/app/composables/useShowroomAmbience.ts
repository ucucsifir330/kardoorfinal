...
export const useShowroomAmbience = () => {
  ...
  watch(
    ...
    (nextMode) => {
      ...
      // Persist only after syncMode has read the stored preference;
      // the immediate run would otherwise overwrite it with the default.
      if (isHydrated.value) saveStoredMode(nextMode);
    },
