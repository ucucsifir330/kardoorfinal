      ...
      >
        ...
        <div
          ...
          :class="{
            ...
            'is-pending': !visibleRows.includes(block.index),
          }"
        >
          <div class="catalog-row-info">
            <transition name="catalog-info-fade">
...
// Formani tarzi staggered giris: kartlar viewport'a girdikce ScrollTrigger.batch
// ile alttan yukari kayarak + fade-in olur. batch, kisa bir aralikta viewport'a
// giren kartlari gruplar → tek seferde stagger'li animasyon (skill onerisi).
// Transition-group hook'lari (tek seferlik, scroll-aware degil) yerine bu kullanilir.
let catalogCardBatch: ScrollTrigger[] = [];

const setupCatalogCardReveal = () => {
  catalogCardBatch.forEach((t) => t.kill());
  catalogCardBatch = [];

  const cards = gsap.utils.toArray<HTMLElement>(".catalog-product");
  if (!cards.length) return;

  // Henuz animasyon almamis (gizli) kartlari baslangic konumuna al.
  cards.forEach((card) => {
    if (card.dataset.revealed) return;
    gsap.set(card, { opacity: 0, y: 48 });
  });

  catalogCardBatch = ScrollTrigger.batch(".catalog-product", {
    start: "top 92%",
    onEnter: (batch) => {
      gsap.to(batch, {
        opacity: 1,
        ...
        stagger: 0.08,
        overwrite: true,
        onComplete: () => batch.forEach((el) => ((el as HTMLElement).dataset.revealed = "1"))
      });
    }
  }) as unknown as ScrollTrigger[];
};

const scheduleCatalogCardReveal = () => {
  nextTick(() => requestAnimationFrame(setupCatalogCardReveal));
};
...
const revealCatalogRow = (rowIndex: number) => {
  ...
  // Yeni kartlar DOM'a girdi → batch reveal'i yeniden kur (yeni kartlari yakalasin).
  if (added) scheduleCatalogCardReveal();
};
